...
export const VGrain: React.FC<{ opacity?: number }> = ({ opacity = 0.08 }) => {
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: 'none', mixBlendMode: 'overlay', opacity }}>
      <svg width={VW} height={VH}>
        ...
        <rect width={VW} height={VH} filter={`url(#vg${seed})`} />
      </svg>
    </AbsoluteFill>
  );
};

export const VFade: React.FC<{ dur: number; inF?: number; outF?: number; children: React.ReactNode }> = ({ dur, inF = 10, outF = 10, children }) => {
